...
- AI 仅指挥自己招募并标记的小队。发现攻击时全队响应最近可见威胁，不会覆盖玩家初始单位或手动招募单位。
...
每个已实现任务都有对应的 `tests/validate_tXX.gd`。例如运行 T25：

```powershell
godot --headless --path . --script res://tests/validate_t25.gd
```
